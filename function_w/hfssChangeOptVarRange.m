...
function hfssChangeOptVarRange(fid, VarList, minList, maxList,Units)
if iscell(VarList) == 0
...
if VarNum ==1
    ...
    fprintf(fid, 'Array("NAME:%s", _\n', VarList{1});
    fprintf(fid, 'Array("NAME:Optimization", "Max:=", "%f%s"))))) \n', maxList(1), Units);
else
    for i = 1:VarNum-1
        fprintf(fid, 'Array("NAME:%s", _\n', VarList{i});
        fprintf(fid, 'Array("NAME:Optimization", "Min:=",  "%f%s")), _\n', minList(i), Units);
        fprintf(fid, 'Array("NAME:%s", _\n', VarList{i});
        fprintf(fid, 'Array("NAME:Optimization", "Max:=", "%f%s")), _\n', maxList(i), Units);
    end
    fprintf(fid, 'Array("NAME:%s", _\n', VarList{VarNum});
